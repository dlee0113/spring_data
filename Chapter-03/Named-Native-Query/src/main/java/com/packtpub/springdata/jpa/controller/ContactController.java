...
import com.packtpub.springdata.jpa.model.Address;
...
public class ContactController {
    ...
    @RequestMapping(value = "/contact/count", method = RequestMethod.POST)
    @ResponseBody
    public long count() {
    ...
    /**
     * Processes contact count requests.
     * @param dto   The search criteria.
     * @return  The number of contacts which match with the given search criteria.
     */
    @RequestMapping(value = "/contact/search/count", method = RequestMethod.POST)
    @ResponseBody
    public long count(@RequestBody SearchDTO dto) {
    ...
    @ResponseBody
    public List<ContactListDTO> findContactForPage(@RequestBody SearchDTO pageSpecification) {
        LOGGER.debug("Finding persons with page specification: {}", pageSpecification);

        List<Contact> contactsInPage = service.findAllForPage(pageSpecification.getPageIndex(), pageSpecification.getPageSize());

        return constructDTOs(contactsInPage);
    }

    /**
     * Processes search contact requests.
     * @param dto   The search criteria.
     * @return  A list of dtos which contains the information of contacts which matched
     *          with the given search criteria.
     */
    @RequestMapping(value = "/contact/search", method = RequestMethod.POST)
    @ResponseBody
    public List<ContactListDTO> search(@RequestBody SearchDTO dto) {
        LOGGER.debug("Performing search for search criteria: {}", dto);

        List<Contact> found = service.search(dto);
        LOGGER.debug("Found {} contacts.", found.size());

        return constructDTOs(found);
    }

    private List<ContactListDTO> constructDTOs(List<Contact> models) {
        List<ContactListDTO> dtos = new ArrayList<ContactListDTO>();

        for (Contact current: models) {
     ...
     * @return  The name of the search result view
     */
    @RequestMapping(value = "/contact/search/{searchTerm}", method = RequestMethod.GET)
    public String showSearchResultPage(@PathVariable("searchTerm") String searchTerm, Model model) {
        LOGGER.debug("Showing the search result page for search term: {}", searchTerm);
        return SEARCH_RESULT_VIEW;
    }

    /**
     * Shows the search result page when search term is not given
     * @return  The name of the search result view.
     */
    @RequestMapping(value = "/contact/search/", method = RequestMethod.GET)
    public String showSearchResultPageWhenSearchTermIsNotGiven() {
        LOGGER.debug("Showing search result page when search term is not given");
        return SEARCH_RESULT_VIEW;
    }
