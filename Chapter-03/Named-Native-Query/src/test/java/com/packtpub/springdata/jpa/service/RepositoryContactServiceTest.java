...
public class RepositoryContactServiceTest {
    ...
    public void countForSearch() {
        ...
        String likePattern = createLikePattern(SEARCH_TERM);
        when(repositoryMock.countContacts(likePattern)).thenReturn(BigInteger.ONE);

        service.count(dto);

        verify(repositoryMock, times(1)).countContacts(likePattern);
        verifyNoMoreInteractions(repositoryMock);
    }
    ...
    public void search() {
        ...
        String likePattern = createLikePattern(dto.getSearchTerm());

        when(repositoryMock.findContacts(eq(likePattern), any(Pageable.class))).thenReturn(expected);

        List<Contact> actual = service.search(dto);

        ArgumentCaptor<Pageable> pageableArgument = ArgumentCaptor.forClass(Pageable.class);

        verify(repositoryMock, times(1)).findContacts(eq(likePattern), pageableArgument.capture());
        verifyNoMoreInteractions(repositoryMock);
    ...
    private String createLikePattern(String searchTerm) {
        StringBuilder likePattern = new StringBuilder();

        likePattern.append(searchTerm.toLowerCase());
        likePattern.append("%");
