...
public class RepositoryContactServiceTest {
    ...
    public void countForSearch() {
        ...
        verify(repositoryMock, times(1)).findByFirstNameStartingWithOrLastNameStartingWith(dto.getSearchTerm(), dto.getSearchTerm());
        verifyNoMoreInteractions(repositoryMock);
    }
    ...
    public void search() {
        ...
        when(repositoryMock.findByFirstNameStartingWithOrLastNameStartingWith(eq(dto.getSearchTerm()), eq(dto.getSearchTerm()), any(Pageable.class))).thenReturn(found);

        List<Contact> actual = service.search(dto);

        ArgumentCaptor<Pageable> pageableArgument = ArgumentCaptor.forClass(Pageable.class);

        verify(repositoryMock, times(1)).findByFirstNameStartingWithOrLastNameStartingWith(eq(dto.getSearchTerm()), eq(dto.getSearchTerm()), pageableArgument.capture());
        verifyNoMoreInteractions(repositoryMock);
